...
import {
  LayoutDashboard, ClipboardList, UserPlus, ClipboardCheck,
  Settings, LogOut, UserCircle, BarChart2,
  CalendarDays,
} from 'lucide-react';

const NAV_ITEMS = [
  { label: 'Dashboard', path: '/chair/dashboard', Icon: LayoutDashboard },
  { label: 'Assigned Requisitions', path: '/chair/assigned-requisitions', Icon: ClipboardList },
  { label: 'Create Committee', path: '/chair/create-committee', Icon: UserPlus },
  { label: 'Schedule Approvals', path: '/chair/schedule-approvals', Icon: CalendarDays },
  { label: 'Evaluations Overview', path: '/chair/evaluations-overview', Icon: ClipboardCheck },
  { label: 'Candidate Status', path: '/chair/candidate-status', Icon: BarChart2 },
  { label: 'Settings', path: '/chair/settings', Icon: Settings },
];

export function ChairLayout({ children }: { children: ReactNode }) {
  const navigate = useNavigate();
  const { pathname } = useLocation();
  const displayName = getSession()?.fullName ?? 'Dr. Ahmed Al-Rashid';
    ...
    <div style={{
      ...
      <aside style={{ width: '260px', backgroundColor: '#002147', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        ...
        <button onClick={() => { clearSession(); navigate('/'); }} style={{
          display: 'flex', alignItems: 'center', gap: '0.7rem', padding: '1rem 1.5rem',
          background: 'none', border: 'none', borderTop: '1px solid rgba(255,255,255,0.1)',
          color: '#FF6C37', fontSize: '0.875rem', fontWeight: '500', cursor: 'pointer', fontFamily: 'inherit',
        }}>
        ...
        <header style={{ height: '64px', backgroundColor: 'white', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', padding: '0 1.5rem', gap: '1.25rem', flexShrink: 0 }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '0.72rem', color: '#FF6C37', fontWeight: '500' }}>Department Chair</div>
            </div>
          </div>
        </header>
